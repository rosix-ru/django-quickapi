...
set BUILDDIR=_build
set ALLSPHINXOPTS=-d %BUILDDIR%/doctrees/%LANGUAGE% -D language=%LANGUAGE% %SPHINXOPTS% .
...
.qhcp project file in %BUILDDIR%/qthelp/%LANGUAGE%, like this:
    ...
    echo.^> assistant -collectionFile %BUILDDIR%\qthelp\%LANGUAGE%\%PROJECT%.ghc
    goto end
)
