# -*- coding: utf-8 -*-
#
# QuickAPI documentation build configuration file, created by
# sphinx-quickstart on Wed Mar 18 12:56:04 2015.
#
...
# If run command sphinx-intl update -d _build/locale -l en,
# then variable `__file__` not exists.
try:
    __file__
except:
    __file__ = sys.argv[0]
...
from quickapi import get_version, get_docs_version
...
title = _('QuickAPI Documentation')
about = _('The Django-application for the fast organization API.')
...
for x in sys.argv:
    if x.startswith('language='):
        language = x.replace('language=', '') or language
...
man_pages = [(master_doc, name, title, [author], 1)]
...
texinfo_documents = [(master_doc, name, title, author, project, about,
                      'Miscellaneous')]
